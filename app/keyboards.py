...
skip_menu_kb = ReplyKeyboardMarkup(
...
async def profile_kb(tg_id):
    ...
    for car in cars:
        ...
        keyboard.add(
            InlineKeyboardButton(
                text=f"{brand} {model}", callback_data=f"car_{brand}_{model}"
            )
        )
    keyboard.add(
        InlineKeyboardButton(text="Список покупок", callback_data="list_purchases")
    )
...
async def get_pagination_keyboard(current_index, total_count, text):
    ...
    if current_index > 0:
        keyboard.add(
            InlineKeyboardButton(text="◀️ Назад", callback_data=f"prev_{current_index}")
        )
    else:
        keyboard.add(
    ...
    keyboard.add(
        InlineKeyboardButton(
            text=f"{current_index + 1}/{total_count}", callback_data="page_info"
        )
    )
    ...
    if current_index < total_count - 1:
        keyboard.add(
            InlineKeyboardButton(text="Вперед ▶️", callback_data=f"next_{current_index}")
        )
    else:
        keyboard.add(
            InlineKeyboardButton(text=" ", callback_data="ignore")
    ...
    keyboard.add(
        InlineKeyboardButton(
            text="Удалить", callback_data=f"delete_{text}_{current_index}"
        )
    )
    keyboard.adjust(3)
...
async def delete_user_reminders_kb(tg_id):
    ...
    for reminder in reminders:
        ...
        keyboard.add(
            InlineKeyboardButton(
                text=text,
                callback_data=f"reminder_{reminder.text}&{reminder.total_date}",
            )
        )
...
async def confirmation_delete_kb(text, current_index):
    ...
    keyboard.add(
        InlineKeyboardButton(text="Назад.", callback_data=f"next_{current_index}")
    )
    return keyboard.adjust(1).as_markup()
