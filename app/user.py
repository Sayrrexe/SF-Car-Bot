...
async def settings_car_callback(callback: CallbackQuery):
    ...
    if cars:
        ...
        await callback.message.answer(
            "У пользователя нет такого авто.", reply_markup=kb.main_kb
        )
...
async def notes_tittle_add(message: Message, state: FSMContext):
    ...
    await message.answer(
        "Введите названия купленного товара для авто:", reply_markup=kb.return_kb
    )
...
async def notes_add_coast(message: Message, state: FSMContext):
    ...
    await message.answer(
        "Введите стоимость этого товара (в вашей валюте, только цифры):",
        reply_markup=kb.return_kb,
    )
...
async def notes_add_final(message: Message, state: FSMContext):
    try:
        ...
        await message.answer(
            "Введен неверный формат цены. Повторите ввод: ", reply_markup=kb.return_kb
        )
        return
    data = await state.get_data()
    await create_notes(data=data)
    await message.answer(
        f"Заметка о покупке товара {data.get('title')} создана.",
        reply_markup=kb.main_kb,
    )
    await state.clear()
...
async def cmd_del_note(message: Message):
    await message.answer(
        ...
        reply_markup=await kb.delete_user_notes_kb(message.from_user.id),
    )


@user.callback_query(F.data.startswith("note_"))
async def notes_delete_callback(
...
async def choose_total_date_reminder(
    ...
    if selected:
        ...
        await callback_query.message.delete()
        await callback_query.message.answer(
            "Введите время в формате 'час, минута': ", reply_markup=kb.return_kb
        )
...
async def add_total_date_reminder(message: Message, state: FSMContext):
    ...
    try:
        ...
        await message.answer(
            "Неверно указано время напоминания", reply_markup=kb.return_kb
        )
        return
...
async def add_text_and_final_reminder(message: Message, state: FSMContext):
    ...
    text = data.get("text")
    await message.answer(
        f"Напоминание о событии {text} добавлено успешно!", reply_markup=kb.main_kb
    )
    await state.clear()
...
async def delete_reminder_cmd(message: Message):
    await message.answer(
        ...
        reply_markup=await kb.delete_user_reminders_kb(tg_id=message.from_user.id),
    )
...
async def delete_reminder_with_callback(callback: CallbackQuery):
    ...
    try:
        ...
        await callback.message.delete()
        await callback.message.answer(
            f"Произошла ошибка, попробуйте ещё раз или напишите /start\nКод ошибки: {e}"
        )
...
async def purchases_cmd(message: Message, state: FSMContext):
    await message.answer(
        "Добавление интересной покупки в ваш личный список, это поможет вам в будущем вспомнить, какой товар вы покупали",
        reply_markup=kb.favorites_kb,
    )
...
async def purchases_cmd(message: Message, state: FSMContext):
    ...
    await message.answer(
        "Введите всё нужную информацию о товаре ( цена будет отдельно ):",
...
async def purchases_add_text(message: Message, state: FSMContext):
    ...
    await message.answer(
        "Введите стоимость этого товара(только цифры):", reply_markup=kb.skip_kb
    )
...
async def purchases_add_price(message: Message, state: FSMContext):
    ...
    await message.answer("Пришлите фото этого товара:", reply_markup=kb.skip_kb)
...
async def purchases_add_image(message: Message, state: FSMContext):
    if message.text == "пропустить":
        ...
        file_name = (
            f"media/purchases/{message.from_user.id}_{message.photo[-1].file_id}.jpg"
        )
        await message.bot.download(
            file=message.photo[-1].file_id, destination=file_name
        )
        await state.update_data(image=file_name)
        await create_purchase(data=await state.get_data())
    await state.clear()
...
@user.message(
    F.text.lower() == "мои товары"
)  # после команды обрабатывает пользователя и генерирует пагинацию
async def purchases_cmd(message: Message, state: FSMContext):
    purchases = await get_user_purchases(message.from_user.id)
    if not purchases:
        await message.answer(
            "У вас нет сохранённых покупок.", reply_markup=kb.favorites_kb
        )
        return
...
async def show_purchase(
    message: Message, purchase, current_index, total_count
):  # универсальная функция для вывода
    text = f"Покупка: {purchase.text}\n" f"Цена: {purchase.price:.2f} ₽\n\n"

    await message.delete()
    if purchase.image:
        try:
            await message.answer_photo(
                photo=FSInputFile(purchase.image, filename="Car"),
                caption=text,
                reply_markup=await kb.get_pagination_keyboard(
                    current_index, total_count, purchase.text
                ),
            )
        except Exception as e:
            await message.answer(
                f"Столкнулись с ошибкой: {e}\n возможно картинка вашего товара была удалена, хотите удалить товар?"
            )
    else:
        await message.answer(
            text,
...
async def pagination_handler(callback_query: CallbackQuery, state: FSMContext):
    ...
    await show_purchase(
        callback_query.message, purchases[new_index], new_index, len(purchases)
    )
...
async def confirm_callback(callback_query: CallbackQuery):
    ...
    index -= 1
    await callback_query.message.answer(
        f"Вы уверенны, что хотите удалить покупку: {text}",
        reply_markup=await kb.confirmation_delete_kb(text, index),
    )
